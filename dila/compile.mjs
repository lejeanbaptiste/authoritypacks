 ...
 *
 * Usage:
 *   node dila/compile.mjs [--persons PATH] [--places PATH] [--districts PATH] [--out DIR]
 */
...
export function compileDila(options = {}) {
  ...
  for (const record of iterateTeiRecords(placesFile, 'place')) {
    const c = placeFromRecord(record, { districtMap });
    if (c) places.push(c);
  }
  ...
  return { persons: personOut.count, places: placeOut.count, outDir: outputDir };
}
...
if (process.argv[1] && path.resolve(process.argv[1]) === fileURLToPath(import.meta.url)) {
  ...
  console.log(`  → ${result.persons} persons, ${result.places} places (${((Date.now() - t0) / 1000).toFixed(1)}s)`);
}
