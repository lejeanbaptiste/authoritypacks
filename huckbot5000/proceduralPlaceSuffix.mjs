/**
 * Procedural translations for place-name + geo-admin suffix office compounds
 * (e.g. 枝江令, 豫章太守, 豫州刺史). These follow a fixed template using a
 * period-aware suffix gloss, so they never need an LLM call.
 *
 * Disambiguation is intentionally conservative: institutional titles that
 * merely end in 令 (尚書令, 黃門令, …) are excluded via blocklists and stem
 * heuristics. When in doubt, the target stays on the LLM path.
 *
 * Place stems are rendered as concatenated toneless pinyin (遼東 → Liaodong),
 * matching leaf-writer's place romanization style.
 */
import { pinyin } from 'pinyin-pro';
import { GEO_ADMIN_PLACE_CAT } from '../norbert/compileOffices.mjs';

const CJK_ONLY = /^[\u4e00-\u9fff]+$/;

/** Full office names that end in 令 but are not place+suffix compounds. */
export const FIXED_LING_OFFICES = new Set([
  '尚書令',
  '中書令',
  '太子家令',
  '太子率更令',
 ...
 * stems. Left for the LLM rather than inventing a place name.
 */
...
export const GEO_ADMIN_SUFFIXES = ['太守', '刺史', '令'];

/** Default suffix gloss when CBDB has no bare row for this dynasty. */
export const DEFAULT_SUFFIX_GLOSS = {
 ...
 * @returns {string | null}
 */
export function romanizePlaceStem(zh) {
 ...
 * @param {string} suffix
 */
function stemEndsWithBannedAdminUnit(stem, suffix) {
...
/**
 * @param {string} zh
 * @returns {{ stem: string, suffix: string, placeCat: string } | null}
 */
export function parseGeoAdminCompound(zh) {
  ...
  for (const suffix of GEO_ADMIN_SUFFIXES) {
    ...
    return { stem, suffix, placeCat: GEO_ADMIN_PLACE_CAT[suffix] };
  }
  return null;
}

/**
 * Build period-aware bare-suffix gloss lookup from CBDB office rows
 * (primaryName exactly 太守 / 刺史 / 令 / 縣令).
 *
 * @param {Array} cbdbOffices
 * @returns {Map<string, Map<string, string>>} suffix -> dynasty -> gloss
 */
export function buildSuffixGlossIndex(cbdbOffices) {
  /** @type {Map<string, Map<string, string>>} */
  const index = new Map();
  for (const suffix of GEO_ADMIN_SUFFIXES) index.set(suffix, new Map());

  for (const row of cbdbOffices) {
    const name = String(row.primaryName ?? '').trim();
    let suffix = null;
    if (GEO_ADMIN_SUFFIXES.includes(name)) suffix = name;
    else if (name === '縣令') suffix = '令';
    if (!suffix) continue;

    const raw = row.metadata?.translation;
    if (!raw || /not yet translated/i.test(raw)) continue;
    const gloss = raw.replace(/\(Hucker\)/gi, '').trim();
    if (!gloss) continue;
    const dynasty = row.metadata?.dynasty ?? '';
    index.get(suffix).set(dynasty, gloss);
  }
  return index;
}
 ...
 */
export function resolveSuffixGloss(suffix, dynasty, glossIndex) {
  const byDynasty = glossIndex.get(suffix);
  if (byDynasty?.has(dynasty ?? '')) return byDynasty.get(dynasty ?? '');
  if (byDynasty?.size === 1) return [...byDynasty.values()][0];
  return DEFAULT_SUFFIX_GLOSS[suffix] ?? null;
}

/**
 * Compose "{role} of {PlacePinyin}".
 *
 * @param {{ stem: string, suffix: string }} parsed
 * @param {string} suffixGloss
 * @param {string} placeRomanization
 */
export function composePlaceSuffixTranslation(parsed, suffixGloss, placeRomanization) {
  return `${suffixGloss} of ${placeRomanization}`;
}

/**
 * @param {object} target Generation target ({ zh, dynasty, … })
 * @param {Map<string, Map<string, string>>} glossIndex
 * @returns {object | null}
 */
export function tryProceduralTranslation(target, glossIndex) {
  const parsed = parseGeoAdminCompound(target.zh);
  if (!parsed) return null;
  const suffixGloss = resolveSuffixGloss(parsed.suffix, target.dynasty, glossIndex);
  if (!suffixGloss) return null;
  const placeRomanization = romanizePlaceStem(parsed.stem);
  if (!placeRomanization) return null;
  const gloss = composePlaceSuffixTranslation(parsed, suffixGloss, placeRomanization);
  return {
    gloss,
    rule: 'place+suffix',
    suffixGloss,
    ...
    placeCat: parsed.placeCat,
  };
}

/**
 * Split resolved targets into procedural (no LLM) and LLM buckets.
 *
 * @param {Array} targets
 * @param {Map<string, Map<string, string>>} glossIndex
 */
export function partitionProceduralTargets(targets, glossIndex) {
  /** @type {Array} */
  const procedural = [];
  /** @type {Array} */
  const llm = [];
  for (const target of targets) {
    const proc = tryProceduralTranslation(target, glossIndex);
